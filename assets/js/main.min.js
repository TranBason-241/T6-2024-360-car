$(document).ready(function () {
  ...
  changeImageSlide = (index) => {
    ...
    document
      .querySelector(".img-model-show")
      .classList.remove("img-model-show");
  ...
  showIcon = (index) => {
    ...
    if (25 < index && index < 46) {
      document.querySelector(".for-mark-2").classList.add("show");
    } else {
      document.querySelector(".for-mark-2").classList.remove("show");
    }
    if (69 < index && index < 90) {
      document.querySelector(".for-mark-3").classList.add("show");
    } else {
      document.querySelector(".for-mark-3").classList.remove("show");
    }
    if (120 < index && index < 141) {
      document.querySelector(".for-mark-4").classList.add("show");
    } else {
      document.querySelector(".for-mark-4").classList.remove("show");
    }
  };
  ...
  marks = document.querySelectorAll(".status__icon");
  marks.forEach((mark) => {
    mark.addEventListener("click", () => {
      var myModal = new bootstrap.Modal(document.getElementById("detail-car"));
      myModal.show();

      document
        .querySelector(".item-detail-show")
        .classList.remove("item-detail-show");

      tartGetContentModalId = mark.getAttribute("modalContent");

      targetContent = document.querySelector(
        `.item-detail-${tartGetContentModalId}`
      );

      console.log(targetContent);
      targetContent.classList.add("item-detail-show");

      setTimeout(() => {
        title = document.querySelector(".modal-header h1")
        if (tartGetContentModalId == "1") {
          console.log("1")
          title.textContent  = "Phần đầu xe"
          var swiper = new Swiper(".swiper-1", {
            pagination: {
              el: ".item-detail-1 .swiper-pagination",
            },
          });
        }
        if (tartGetContentModalId == "2") {
            title.textContent  = "Phần thân xe"
          document.querySelector(".modal-header h1").innerHTML = "Phanaf duoi";
          var swiper = new Swiper(".swiper-2", {
            pagination: {
              el: ".item-detail-2 .swiper-pagination",
            },
          });
        }
        if (tartGetContentModalId == "3") {
            title.textContent  = "Phần đuôi xe"
          var swiper = new Swiper(".swiper-3", {
            pagination: {
              el: ".item-detail-3 .swiper-pagination",
            },
          });
        }
        if (tartGetContentModalId == "4") {
           title.textContent  = "Phần nội thất"
          var swiper = new Swiper(".swiper-4", {
            pagination: {
              el: ".item-detail-4 .swiper-pagination",
            },
          });
        }
      }, 500);
